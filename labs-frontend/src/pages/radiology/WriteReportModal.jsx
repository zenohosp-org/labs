...
import { radiologyApi, referenceRangeApi } from "@/api/labsClient";
...
/**
 * Write a radiology / lab report. Free-text findings + observation remain the
 * primary surface (no behavioural change), but a "Quick Value Match" panel
 * sits above them:
 *
 *   1. Pathologist types a measured value (with auto-filled test name).
 *   2. The reference-range match endpoint is called (debounced, server-side).
 *   3. We render the matched band + LOW/NORMAL/HIGH flag inline.
 *   4. "Insert into Findings" appends a formatted line to the findings textarea.
 *
 * Catalogue is editable from Settings → Reference Ranges. When no band is
 * configured for a (test, sex, age) tuple the matcher returns 204 and we tell
 * the user how to add one.
 */
function WriteReportModal({ order, onClose, onSaved }) {
    ...
    // Debounce: kick off a match when both test name and value are present.
    useEffect(() => {
    ...
    const insertMatchLine = () => {
        ...
        // Reset value so the user can move on to the next test
        setMatchValue("");
        setMatchResult(null);
    };
        ...
        <div className="hms-rad-modal-overlay">
            <div className="hms-rad-modal">
                ...
                <form onSubmit={handleSubmit} className="hms-rad-modal__body">
                    {/* Quick value match — optional but speeds up common cases. */}
